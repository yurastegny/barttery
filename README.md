...
> iPhone and Apple Watch remain visible for up to 24 hours after the last successful sync — Apple does not allow apps to force a connection, so the app waits for the device to become available on its own. To see when the last sync happened, expand the device row in the menu.
